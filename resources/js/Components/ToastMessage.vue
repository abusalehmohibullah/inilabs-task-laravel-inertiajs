...
watchEffect(async () => {
    style.value = page.props.toast?.style || 'success';
    message.value = page.props.toast?.message || '';
    show.value = true;

    // Automatically hide the toast after 5000 milliseconds (5 seconds)
    setTimeout(() => {
        show.value = false;
    }, 3000);
});
...
<template>
    <div class="fixed top-1 right-1 z-10">
        <transition name="toast" mode="out-in">
            <div v-if="show && message" :class="{ 'bg-emerald-700': style == 'success', 'bg-red-700': style == 'danger' }">
